  ...
  function consume(events) {
    for (const e of events) {
      ...
      switch (ev.type) {
        ...
        case "selfDraw": {
          const ctx = p.context;
          const score2 = p.score;
          const tiles = (p.concealed ?? []).map(label).join(" ");
          const melds = (p.melds ?? []).map((m) => `[${m.tiles.map(label).join(" ")}]`).join(" ");
          overlayHtml = `<h2>${ctx.seat === HUMAN ? "YOU WIN" : who(ctx.seat) + " wins"} \u2014 ${score2.faan} faan ${ctx.selfDraw ? "\u81EA\u6478 self-draw" : "\u98DF\u7CCA" + (ctx.from === HUMAN ? " \u2014 off YOUR discard" : "")}</h2>
          <div class="tilesrow">${tiles} ${melds}</div>
          <div class="awards">${score2.awards.map((a) => `${AWARD_LABELS[a.id] ?? a.id} <b>${a.faan}</b>`).join(" \xB7 ")}</div>`;
          break;
        }
        case "exhaustiveDraw":
