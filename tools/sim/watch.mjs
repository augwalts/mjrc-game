...
var bracketFor = (faan) => {
  ...
  for (const row2 of LIU_BRACKETS) if (capped <= row2.maxFaan) return row2;
...
function rankDiscards(v, cfg) {
  ...
  const scored = candidates.map((tile) => {
    ...
    const score3 = -distance * profile.discardDistanceWeight - routeDistance * profile.discardRouteWeight * 0.5 + (restricts ? fits ? -profile.discardRouteWeight : profile.discardRouteWeight : 0) - danger * profile.discardSafetyWeight;
...
// tools/sim/watch.ts
var SEATS = [0, 1, 2, 3];
var NAMES = ["East-seat", "Ah Ming", "Kai", "Suki"];
var args = process.argv.slice(2);
var flag = (n) => {
  const i = args.indexOf(n);
  return i >= 0 ? args[i + 1] ?? "" : null;
};
var SEED = Number(flag("--seed") ?? 42);
var RUNS = Number(flag("--seeds") ?? 1);
var QUIET = args.includes("-q") || RUNS > 1;
var glyph = (t) => TILE_NAMES[t] ?? `?${t}`;
var row = (ts) => [...ts].sort((a, b) => a - b).map(glyph).join(" ");
...
function autopsy(state) {
  ...
  for (const s of SEATS) {
    ...
    const waits = lt.tiles.map((w) => `${glyph(w.tile)}\xD7${Math.max(0, 4 - Math.min(4, vis[w.tile] + (c[w.tile] ?? 0)))}`).join(" ");
    const melds = seat.melds.map((m) => `[${m.tiles.map(glyph).join("")}${m.concealed ? "\xB7concealed" : ""}]`).join(" ");
    console.log(
      `  ${WIND_NAMES[seat.wind]} ${NAMES[s].padEnd(9)} ${lt.distance < 0 ? "COMPLETE?!" : lt.distance === 0 ? "READY" : lt.distance + " away"}  hand: ${row(all)} ${melds}` + (lt.distance === 0 ? `  waiting: ${waits || "(dead wait \u2014 every winning tile already visible)"}` : "")
    );
  }
}
...
hand ${state.handIndex}: EXHAUSTIVE DRAW \u6D41\u5C40`);
          ...
          for (const s of SEATS) {
            ...
            if (lt.distance === 0) {
              anyReady = true;
              if (lt.total > 0) allDead = false;
            }
          }
          if (anyReady) tally2.readyAtDraw++;
          if (anyReady && allDead) tally2.deadWaitDraws++;
          if (!QUIET) autopsy(state);
        } else {
          tally2.wins++;
          if (p.outcome === "selfDraw") tally2.selfDraws++;
          if (typeof p.faan === "number") tally2.faans.push(p.faan);
          if (!QUIET) console.log(`
hand ${state.handIndex}: WIN by ${NAMES[p.winner] ?? p.winner} \u2014 ${p.faan} faan (${p.outcome})`);
        }
      }
      if (e.type === "refusedWin") {
        tally2.refused++;
        if (!QUIET) console.log(`  !! refused win (under the 3-faan floor) seat ${e.actor}`);
      }
      if (e.type === "claimed") {
        tally2.claims++;
        if (!QUIET) console.log(`  claim: seat ${e.actor} ${String(e.payload.kind ?? "")} ${glyph(e.payload.tile ?? -1)}`);
      }
...
var tally = { hands: 0, draws: 0, wins: 0, selfDraws: 0, refused: 0, claims: 0, deadWaitDraws: 0, readyAtDraw: 0, faans: [] };
var t0 = Date.now();
for (let i = 0; i < RUNS; i++) playMatch(SEED + i, tally);
var ms = Date.now() - t0;
console.log(`
\u2550\u2550 ${RUNS} match(es), ${tally.hands} hands in ${ms}ms \u2014 ${(tally.hands / (ms / 1e3)).toFixed(1)} hands/sec \u2550\u2550`);
