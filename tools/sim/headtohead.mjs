...
function assessRoutes(shape, rules, profile2 = DEFAULT_PROFILE) {
  ...
  for (const route of ROUTES) {
    ...
    let score3 = faan * profile2.faanWeight - distance * profile2.routeDistanceWeight - surplus * profile2.offRouteWeight;
...
function chooseRoute(shape, rules, profile2 = DEFAULT_PROFILE) {
  const all = assessRoutes(shape, rules, profile2);
  let best = all[0];
...
function rankDiscards(v, cfg) {
  ...
  const chosen = chooseRoute(shape, cfg.ruleset, profile2);
  const visible = visibleCounts(v);
  let foldFactor = 0;
  let threats = null;
  if (profile2.threatSensitivity > 0) {
  ...
  const scored = candidates.map((tile) => {
    ...
    const score3 = -distance * profile2.discardDistanceWeight - routeDistance * profile2.discardRouteWeight * 0.5 + (restricts ? fits ? -profile2.discardRouteWeight : profile2.discardRouteWeight : 0) - danger * profile2.discardSafetyWeight - threatDanger * foldFactor * profile2.threatSensitivity;
...
function claimContext(v, cfg) {
  const profile2 = profileOf(cfg);
  const shape = shapeOf(v);
  const route = chooseRoute(shape, cfg.ruleset, profile2);
...
function assessClaim(v, option, cfg, context) {
  ...
  const afterRoute = chooseRoute(after, cfg.ruleset, profile2);
  if (!afterRoute.feasible || afterRoute.faan < cfg.ruleset.minimumFaan) {
    return { ...dead, reason: "offRoute", faanCeiling: ceiling };
  }
  if (afterRoute.score < before.route.score - profile2.claimRouteTolerance) {
    return { ...dead, reason: "offRoute", faanCeiling: ceiling };
  }
...
function playMatch(config, decide) {
  ...
  for (let guard = 0; guard < 2e5; guard++) {
    for (const e of events) {
      ...
      if (e.type === "winOnDiscard" || e.type === "selfDraw") {
        ...
        const score3 = e.payload.score;
        for (const a of score3?.awards ?? []) r.patterns[a.id] = (r.patterns[a.id] ?? 0) + 1;
      }
    }
