...
function claimSupplyCredit(route, pairs, profile2) {
  if (route.orphans) return 0;
  const rate = route.pungs || route.honoursOnly ? 1 : route.suit !== null ? 0.5 : 0.25;
  return profile2.claimSupplyWeight * rate * pairs;
}
function routeValue(faan, distance, rules, profile2, urgency) {
  const cv = profile2.chipValuation;
  if (cv <= 0) return faan;
  ...
  const decay = Math.max(0.15, profile2.routeDecay * (1 - profile2.urgencyWeight * urgency));
...
function assessRoutes(shape, rules, profile2 = DEFAULT_PROFILE, table = null) {
  ...
  for (const route of ROUTES) {
    ...
    const credit = Math.min(
      ...
      claimSupplyCredit(route, convertiblePairs(routeCounts(route, c)), profile2)
    );
    const effDistance = Math.max(0, distance) - credit;
    let score2 = routeValue(faan, effDistance, rules, profile2, urgency) * profile2.faanWeight - effDistance * profile2.routeDistanceWeight * (route.orphans ? ORPHANS_DISTANCE_TAX : 1) - surplus * profile2.offRouteWeight + // 上家 as supply line (owner, 2026-08-27): a suit route lives or dies on
    // whether the seat before you is feeding that suit or hoarding it.
    (route.suit !== null ? leftFeed(shape, route.suit) * profile2.leftFeedWeight : 0) - // SUIT SUPPLY: a route into a suit the table is eating — a collector
...
function chooseRoute(shape, rules, profile2 = DEFAULT_PROFILE, table = null) {
  const all = assessRoutes(shape, rules, profile2, table);
  let best = all[0];
...
function rankDiscards(v, cfg) {
  const profile2 = profileOf(cfg);
  const shape = shapeOf(v);
  const threats = tableRead(v, cfg);
  const chosen = chooseRoute(shape, cfg.ruleset, profile2, threats);
  ...
  const bankedShort = profile2.keepPayableWeight > 0 ? Math.max(0, cfg.ruleset.minimumFaan - fallbackFaan(shape, cfg.ruleset)) : 0;
  ...
  if (tied.length > 1) {
    for (const d of tied) {
      ...
      c[d.tile]++;
      d.score += d.outs * profile2.discardOutsWeight;
    }
    scored.sort((a, b) => b.score - a.score || a.tile - b.tile);
  }
  return scored;
...
function claimContext(v, cfg) {
  const profile2 = profileOf(cfg);
  const shape = shapeOf(v);
  const route = chooseRoute(shape, cfg.ruleset, profile2, tableRead(v, cfg));
...
function assessClaim(v, option, cfg, context) {
  ...
  const shapeProfile = { ...profile2, chipValuation: 0 };
  ...
  if (afterRoute.score < beforeRoute.score - profile2.claimRouteTolerance) {
    return { ...dead, reason: "offRoute", faanCeiling: ceiling };
  }
  ...
  const speedBar = (option.kind === "kong" ? 0 : profile2.claimSpeedGain) * (2 - profile2.aggression);
  const riskBar = shortfall * profile2.claimFallbackWeight;
  ...
  const score2 = afterRoute.score - before.route.score + gain * 1.4 * profile2.aggression + (option.kind === "kong" ? 0.6 : 0) + (afterRoute.attainable - cfg.ruleset.minimumFaan) * 0.15 - shortfall * profile2.claimFallbackWeight;
  return {
    option,
    reason: "accepted",
    faanCeiling: ceiling,
...
function startMatch(config) {
  ...
  const chips2 = config.startingChips ?? 0;
...
var RULES = process.argv[6] ? ruleset(process.argv[6]) ?? MJRC_STANDARD : MJRC_STANDARD;
