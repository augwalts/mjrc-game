...
function assessClaim(v, option, cfg, context) {
  ...
  const bar = option.kind === "kong" ? 0 : profile2.claimSpeedGain;
  if (gain < bar * (2 - profile2.aggression)) {
    return {
      option,
      reason: "tooSlow",
      faanCeiling: ceiling,
  ...
  const score2 = afterRoute.score - before.route.score + gain * 1.4 * profile2.aggression + (option.kind === "kong" ? 0.6 : 0) + (afterRoute.attainable - cfg.ruleset.minimumFaan) * 0.15;
  return {
    option,
    reason: "accepted",
    faanCeiling: ceiling,
