...
// .wrangler/tmp/pages-DgAPtU/functionsWorker-0.9365992511708049.mjs
...
var onRequestPost = /* @__PURE__ */ __name2(async (ctx) => {
  ...
  try {
    await ctx.env.DB.prepare(
      ...
      f.context ? JSON.stringify(f.context).slice(0, 2e4) : null,
      Date.now()
    ).run();
  } catch (e) {
...
var MAX_BODY2 = 512 * 1024;
var isNum = /* @__PURE__ */ __name2((v) => typeof v === "number" && Number.isFinite(v), "isNum");
var isStr = /* @__PURE__ */ __name2((v) => typeof v === "string" && v.length > 0, "isStr");
function invalid(m) {
  ...
  if (m.actionsGz !== void 0 && (typeof m.actionsGz !== "string" || m.actionsGz.length > 4e5)) {
    return "actionsGz";
  }
  if (m.moves !== void 0 && (!Array.isArray(m.moves) || m.moves.length > 4e3)) return "moves";
  return null;
}
__name(invalid, "invalid");
__name2(invalid, "invalid");
var onRequestPost2 = /* @__PURE__ */ __name2(async (ctx) => {
  const body = await readJsonBody(ctx.request, MAX_BODY2);
  if (body instanceof Response) return body;
  const m = body;
  const bad = invalid(m);
  ...
  stmts.push(ctx.env.DB.prepare(
    ...
    m.fed ?? 0,
    m.drawnHands ?? 0,
    JSON.stringify(m.seatWins),
    m.matchRate ?? null,
    m.meanGap ?? null,
    m.movesGraded ?? 0,
    m.client ? JSON.stringify(m.client) : null,
    m.actionsGz ?? null,
    now
  ));
  ...
  for (const v of m.moves ?? []) {
    ...
    stmts.push(ctx.env.DB.prepare(
      ...
      v.turn,
      v.kind,
      String(v.played ?? "").slice(0, 40),
      String(v.enginePick ?? "").slice(0, 40),
      isNum(v.gap) ? v.gap : 0,
      isNum(v.top1MinusTop2) ? v.top1MinusTop2 : 0,
...
__name(passwordsMatch, "passwordsMatch");
__name2(passwordsMatch, "passwordsMatch");
var unauthorized = /* @__PURE__ */ __name2((message, status = 401) => new Response(message, {
  status,
  headers: {
...
// .wrangler/tmp/bundle-HFTMtr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-HFTMtr/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9365992511708049.js.map
